// Metal. GGML block-quantized weight matmul for the GpuGeneric backend.
// Activation is FP32 (like MpsBackend's equivalent custom kernel — see
// src/metal/ggml_quant_metal.hpp, which this file's dequant math mirrors
// verbatim); the weight buffer holds raw GGML block bytes, dequantized
// per-element inline rather than expanded to a dense float buffer up front.
//
// Same 1D column-tiling dispatch shape as matmul.metal: campello_gpu's
// Metal dispatch uses the pipeline's threadExecutionWidth as the
// threadgroup size, so we gate to the first tileWidth threads and have
// each compute one output column. Unlike matmul.metal there is no
// separate batch dispatch dimension — `rows` already folds batchCount*M
// (mirroring MpsBackend's kernel, where the weight matrix is shared
// across the batch and only the activation/output are batch-indexed).
...
kernel void computeMain(const device float *activation [[buffer(0)]],
                         const device uchar *weightRaw [[buffer(1)]],
                         ...
                         uint3 localId [[thread_position_in_threadgroup]])
{
    ...
    {
        float a = activation[row * K + kk];
        float w = dequantizedWeight(weightRaw, params.ggmlType, weightBase + kk);
        sum += a * w;
    }
    outputBuf[row * params.n + n] = sum;
}
