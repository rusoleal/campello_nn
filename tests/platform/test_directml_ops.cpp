...
// Mirrors tests/universal/test_cpu_ops.cpp, but runs against the DirectML
// backend (DeviceType::Gpu) to check the IR-to-DML-operator mapping is
// correct, not just that it compiles. Mirrors tests/platform/test_mps_ops.cpp
// op-for-op (same hand-computed expected values/tolerances) so any
// CPU/MPSGraph/DirectML divergence shows up as a failing test, not a guess.

TEST(DirectMlOps, Add)
{
...
TEST(DirectMlOps, AddBroadcastRowVector)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
...
TEST(DirectMlOps, MulBroadcastColumnVector)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
    auto a = builder.input("a", {cnn::DataType::Float32, {2, 3}});
...
TEST(DirectMlOps, ResizeBilinearAlignCorners)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
...
TEST(DirectMlOps, ResizeNearestAlignCorners)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
    auto x = builder.input("x", {cnn::DataType::Float32, {1, 1, 2, 2}});
...
TEST(DirectMlOps, ResizeNearestFloorAsymmetric)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
    auto x = builder.input("x", {cnn::DataType::Float32, {1, 1, 2, 2}});
...
TEST(DirectMlOps, GatherUint32Indices)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
    auto data = builder.input("data", {cnn::DataType::Float32, {4, 2}});
