...
// Mirrors tests/universal/test_cpu_float16_ops.cpp / test_mps_float16_ops.cpp,
// but runs against the real DirectML backend, which computes Float16 natively
// (DML_TENSOR_DATA_TYPE_FLOAT16) — no Float32 decode step, same as MPSGraph.

TEST(DirectMlFloat16Ops, Add)
{
...
TEST(DirectMlFloat16Ops, MatMul)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
...
TEST(DirectMlFloat16Ops, LayerNorm)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
