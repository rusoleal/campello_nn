...
// Mirrors tests/platform/test_mps_quantization_ops.cpp against the real
// DirectML backend (DML_OPERATOR_ELEMENT_WISE_QUANTIZE_LINEAR/DEQUANTIZE_LINEAR).

TEST(DirectMlQuantizationOps, QuantizeLinear)
{
...
TEST(DirectMlQuantizationOps, DequantizeLinear)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
...
TEST(DirectMlQuantizationOps, QuantizedMatmul)
{
    auto context = makeGpuContext();
    cnn::GraphBuilder builder(context);
